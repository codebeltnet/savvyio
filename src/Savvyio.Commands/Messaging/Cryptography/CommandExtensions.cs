...
namespace Savvyio.Commands.Messaging.Cryptography
{
    ...
    public static class CommandExtensions
    {
        ...
        /// <param name="marshaller">The <see cref="IMarshaller"/> that is used when converting the <paramref name="command"/> into an <see cref="ISignedMessage{T}"/>.</param>
        ...
        /// </exception>
        public static ISignedMessage<T> Sign<T>(this IMessage<T> command, IMarshaller marshaller, Action<SignedMessageOptions> setup) where T : ICommand
        {
            Validator.ThrowIfNull(command);
            Validator.ThrowIfNull(marshaller);
            Validator.ThrowIfInvalidConfigurator(setup, out var options);
            var message = marshaller.Serialize(command);
