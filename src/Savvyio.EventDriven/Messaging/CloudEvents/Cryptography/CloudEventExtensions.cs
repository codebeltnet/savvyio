...
namespace Savvyio.EventDriven.Messaging.CloudEvents.Cryptography
{
    ...
    public static class CloudEventExtensions
    {
        ...
        /// <param name="marshaller">The <see cref="IMarshaller"/> that is used when converting the <paramref name="cloudEvent"/> into an <see cref="ISignedMessage{T}"/>.</param>
        ...
        public static void CheckSignature<T>(this ISignedCloudEvent<T> cloudEvent, IMarshaller marshaller, Action<SignedMessageOptions> setup) where T : IIntegrationEvent
        {
            Validator.ThrowIfNull(cloudEvent);
            Validator.ThrowIfNull(marshaller);
            Validator.ThrowIfInvalidConfigurator(setup, out _);
            var baseCloudEvent = Sign(cloudEvent.Clone(() => new CloudEvent<T>(cloudEvent, cloudEvent.SpecVersion)) as ICloudEvent<T>, marshaller, setup);
